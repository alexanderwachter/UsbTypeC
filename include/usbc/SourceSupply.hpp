 ...
 * supply onto VBUS (sourceVbus()). Only ports that source power need
 * to provide it.
 *
 ...
 * policy engine answers with error recovery. A new setOutput()
 * replaces the previous target.
 *
 * On a Request the sequence is: send Accept, wait tSrcTransition,
 * setOutput(), await the settled callback, send PS_RDY.
 *
